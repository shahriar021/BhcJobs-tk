...
module.exports = function withRootBuildGradle(config) {
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      // Fix AGP version
      contents = contents.replace("classpath('com.android.tools.build:gradle')", "classpath('com.android.tools.build:gradle:8.9.1')");

      // Add expoProvidesDefaultConfig if not present
